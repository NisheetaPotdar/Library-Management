...
if ($conn->connect_error) {
    ...
    if ($result_email->num_rows > 0) {
        echo "<script>alert('Error: Email address already exists!'); window.location.href = 'signup_process.php';</script>";
    } 
    elseif ($result_name->num_rows > 0) {
        echo "<script>alert('Error: Name already exists!'); window.location.href = 'signup_process.php';</script>";    
    } 
    else {
        ...
        if ($conn->query($sql) === TRUE) {
            echo "<script>alert('Signup Successfully.'); window.location.href = 'signup_process.php';</script>";
        } else {
